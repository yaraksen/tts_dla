...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        attn: Tensor = q @ k.transpose(1, 2) / self.temperature
        if mask is not None: attn.masked_fill_(mask, -torch.inf)
...
class MultiHeadAttention(nn.Module):
    ...
    def reset_parameters(self):
        # xavier distribution initialization better than normal
        for _, l in self.named_modules():
...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, d_in, d_hid, fft_conv1d_kernel: Tuple, fft_conv1d_padding: Tuple, dropout=0.1):
        super().__init__()
